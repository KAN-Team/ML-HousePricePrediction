...
from HousePricePrediction.Regression.RegressionDataPreProcessing import start_preprocessing
...
ridge_start_time = time.time()  # alarm start time

model = RidgeCV(alphas=range(1, 100))
model.fit(X_train, Y_train)
pickle.dump(model, open('SavedData/regression_ridge_model.sav', 'wb'))
prediction = model.predict(X_test)

ridge_end_time = time.time()    # alarm finish time
